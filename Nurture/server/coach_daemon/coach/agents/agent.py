...
class Agent(object):
    ...
    def act(self, phase=RunPhase.TRAIN):
        ...
        return self.act_process_reward(result, phase)

    def act_get_action(self, phase=RunPhase.TRAIN):
    ...
    def act_process_reward(self, result, phase=RunPhase.TRAIN):
            ...
            if self.tp.agent.collect_new_data:
                if self.tp.agent.step_until_collecting_full_episodes:
                    ...
                    while step < self.tp.agent.num_consecutive_playing_steps or self.memory.get_episode(-1).length() != 0:
                        #self.act()
                        action = self.act_get_action()
                        yield action
                        result = self.env.step(action)
                        self.act_process_reward(result)
