"""
An extention of `CoachA3CAgent`. This agent punishes a notification being ignored
"""
...
kIgnoreTimeoutSeconds = 15 * 60  # 15 minutes
...
class CoachA3CPunishIgnoreAgent(BaseAgent):
    ...
    def get_policy_name(cls):
        return 'coach-a3c-punish-ignore'
    ...
    def _process_state_and_get_action(self, state):
        if self.gym_stage == CoachA3CPunishIgnoreAgent.GYM_STAGE_WAIT_REWARD:
        ...
        if send_notification:
            # if a new notification is sent, it means any pending notification is ignored
            if self.pending_notification:
                self.last_reward += kIgnoreReward

            # flag pending notification
            self.pending_notification = True

            # remember when we send notification
            self.last_notification_time = datetime.datetime.now()

        self.gym_stage = CoachA3CPunishIgnoreAgent.GYM_STAGE_WAIT_REWARD
    ...
    def _process_reward(self, reward):
        ...
        if reward != 0.:
            # clear the pending notification flag if a response is received
            self.pending_notification = False
        elif self.pending_notification:
            # check if notification is timed out
            dt = datetime.datetime.now() - self.last_notification_time
            if dt.total_seconds() > kIgnoreTimeoutSeconds:
                reward = kIgnoreReward
                self.pending_notification = False

        # accumulate the rewards anyway
        self.last_reward += reward
        
        self.gym_stage = CoachA3CPunishIgnoreAgent.GYM_STAGE_WAIT_STATE
